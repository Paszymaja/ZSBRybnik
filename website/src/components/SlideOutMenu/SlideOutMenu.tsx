  ...
  return (
    ...
    >
      <SlideOutMenuHeightFixer isDarkTheme={isDarkTheme}>
        {routes &&
          Object.keys(routes).map((key: string, index: number): JSX.Element => {
            if (
              Array.isArray(
              ...
              const categoryChildren: JSX.Element[] =
                (((routes as any)[key] as ExtendedCategory).children).map(
                  (
              ...
              const category: JSX.Element = <SlideOutMenuCategory
                title={title}
                onlyForMobile={onlyForMobile}
                key={index}
              >
              ...
              const { isInnerLink, title, route, onlyForMobile }: Subpage =
                (routes as any)[key] as Subpage;
              if (isInnerLink) {
                return <InnerLink
                  title={title}
                  route={route}
