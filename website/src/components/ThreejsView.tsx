import React, { FC, useEffect, useRef, MutableRefObject, useContext, useState } from 'react';
...
const ThreejsView: FC<ThreejsViewProps> = (props: ThreejsViewProps): JSX.Element => {
  ...
  const createDirectionalLight: CreateDirectionalLight = (scene: THREE.Scene, lightPosition: [number, number, number]): void => {
    const directionalLight: THREE.DirectionalLight = new THREE.DirectionalLight(0xffffff, 100);
  ...
  useEffect(() => {
    ...
    const camera: THREE.PerspectiveCamera = new THREE.PerspectiveCamera(90, window.innerWidth / window.innerHeight);
    ...
    const resizeRendererToDisplaySize: ResizeRendererToDisplaySize = (renderer: THREE.WebGLRenderer): boolean => {
