...
const PostPage: FC<PostPageProps> = (): JSX.Element => {
  ...
  const parsedLocationId: string | string[] | null | undefined = parsedLocation.id;
  const isParsedLocationValid: boolean = parsedLocationId === undefined || parsedLocationId === null ? false : true;
  ...
  useEffect((): void => {
    const makePostRequest: MakePostRequest = (): void => {
      ...
      const tryRequest: TryRequest = async (): Promise<void> => {
        try {
          const res: Response = await fetch(`http://${window.location.hostname}:5002/api/get-post?id=${parsedLocationId}`, {
            method: 'GET',
            signal: signal
          });
  ...
  const codeBlockValue: string = `${window.location.origin}${window.location.pathname}&id=numerPosta`;
  const firstLineErrorText: string = "Nie jesteśmy w stanie wyświetlić zawartości, jeśli nie podałeś parametru określającego numer posta. Proszę uzupełnij URL o ten parametr.";
  ...
  return (
    <Page title={postTitle}>
      <h2>{isParsedLocationValid === false ? "Podaj parametr id, żeby przenieść się do odpowiedniego posta:" : postTitle}</h2>
      <Section>
